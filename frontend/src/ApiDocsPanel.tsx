...
const modules: ApiModule[] = [
  ...
  },
  {
    id: 'admin', name: 'Administração', endpoints: [
      ...
      { method: 'POST', path: '/api/v1/admin/users/batch', title: 'Criação em lote de usuários', description: 'Cria múltiplos usuários em uma única requisição. Cada usuário é criado individualmente; erros são reportados por índice sem interromper os demais.', auth: 'Obrigatória', permission: 'ADMIN', body: [field('users', 'Array<CreateUserRequest>', true, 'Lista de 1 a 500 usuários (username, name, email, password).'), field('roles', 'Array<String>', false, 'Roles aplicadas a todos. Padrão: ["USER"].')], response: '{ total, created, errors: [{ index, username, code, message }] }', responseExample: { total: 3, created: 2, errors: [{ index: 1, username: 'repetido', code: 'USERNAME_TAKEN', message: 'Nome de usuário já existe: repetido' }] }, statuses: commonErrors },
      ...
      { method: 'GET', path: '/api/v1/admin/monitoring/messages-timeseries', title: 'Série temporal de mensagens', description: 'Retorna contagem temporal de mensagens agrupadas por dia, mês ou ano.', auth: 'Obrigatória', permission: 'ADMIN', query: [field('period', 'String', false, 'DAYS_7, DAYS_30, DAYS_90, MONTHS_12 ou YEARS.')], response: '{ granularity, period, totalMessages, averageMessages, peakMessages, points: [...] }', statuses: commonErrors },
